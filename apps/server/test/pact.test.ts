...
import { catalog } from '@kingmakers/data';
...
describe('Secret Pact Betrayal Evaluation', () => {
  it('should penalize the proposer if pact is not honored', () => {
    let state: GameState = {
      ...
      round: {
        ...
        campaignTurnOrder: ['player-1' as PlayerId, 'player-2' as PlayerId],
        camps: {},
  ...
  it('should not penalize if the pact is honored via conditional support', () => {
    ...
    // 1. 밀약 제안 및 수락
    state = reduce(state, { type: 'proposeSecretPact', actor: 'player-1' as PlayerId, counterparty: 'player-2' as PlayerId, promise: { kind: 'supportCandidate', candidateId: 'candidate-01' } }, catalog).state as GameState;
    state = reduce(state, { type: 'acceptSecretPact', actor: 'player-2' as PlayerId, proposer: 'player-1' as PlayerId }, catalog).state as GameState;

    // 2. 조건지지 실행
    state = reduce(state, { type: 'conditionalSupport', actor: 'player-1' as PlayerId, candidateId: 'candidate-01' }, catalog).state as GameState;

    // 3. 점수 정산
    state = { ...state, phase: 'roundScoring' };
    state = reduce(state, { type: 'scoreRound' }, catalog).state as GameState;
