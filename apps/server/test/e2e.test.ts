...
describe('부록 A-17 & A-22: 호스트 겸 참가, 관전자 정원, 공개방 목록, 방 나가기', () => {
  ...
  it('대기 중인 참가자(비호스트)는 나갈 수 있고, 방은 그대로 남는다', async () => {
    clearRooms();
    const host = await connect();
    const created = await emitAck<{ roomId: string; hostToken: string }>(host, 'room:create', { name: '호스트', hostJoinsAsPlayer: true });
    const { roomId } = created;
...
describe('부록 A-23: AI 참가자 옵션', () => {
  ...
  it('호스트는 대기실에서 AI 참가자를 추가/제거할 수 있고, 비호스트는 조작할 수 없다', async () => {
    ...
    expect(add.roomState.players).toHaveLength(2); // 호스트(자동 참가) + AI 1
    expect(add.roomState.players[1]).toMatchObject({ name: 'AI 1', isAi: true, ready: true });
