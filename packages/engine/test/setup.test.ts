...
describe('setupGame 결정성 (§28: seed가 같으면 setup 결과가 같다)', () => {
  it('같은 seed + 같은 플레이어 입력이면 완전히 동일한 GameState를 만든다', () => {
    const a = setupGame({ seed: 12345, players: players(4) });
    const b = setupGame({ seed: 12345, players: players(4) });
    expect(a).toEqual(b);
  });

  it('다른 seed면 카드 셔플 순서가 달라진다', () => {
    const a = setupGame({ seed: 1, players: players(4) });
    const b = setupGame({ seed: 2, players: players(4) });
...
describe('인원별 설정표 (§3)', () => {
  ...
  it('4~7인 밖의 인원수는 지원하지 않는다', () => {
    ...
    expect(() => setupGame({ seed: 1, players: players(3) })).toThrow();
  });
});

describe('setupGame 초기 상태', () => {
  it('좌석·시작 자원·phase가 올바르게 초기화된다', () => {
    const state = setupGame({ seed: 42, players: players(4) });
  ...
  it('플레이어마다 서로 다른 비밀 의제를 1장씩 배분한다 (부록 A-7)', () => {
    const state = setupGame({ seed: 7, players: players(7) });
